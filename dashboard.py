...
if market_choice == "Borsa İstanbul (BIST)":
    ...
    pb_limit = st.sidebar.slider("Maksimum PD/DD", 0.5, 20.0, 5.0, step=0.5, help="Piyasa Değeri / Defter Değeri: Şirketin piyasa değerinin özsermayesine (muhasebe değerine) oranıdır. Özellikle banka, holding ve ağır sanayi şirketlerinin aşırı fiyatlanıp fiyatlanmadığını gösterir.")
    ...
    pb_limit = st.sidebar.slider("Maksimum PD/DD", 0.5, 30.0, 10.0, step=0.5, help="Price to Book (P/B) Ratio: ABD piyasasında teknoloji şirketlerinin defter değeri düşük, marka/patent değerleri yüksek olduğu için PD/DD oranları BIST'e göre çok daha yüksek çıkabilir.")
...
def fetch_tradingview_data(market_code, filter_ops):
    ...
    payload = {
        "columns": ["name", "close", "price_earnings_ttm", "return_on_equity", "dividend_yield_recent", "debt_to_equity", "market_cap_basic", "price_book_ratio", "sector"],
...
def fetch_tefas_data():
    tefas = Crawler()
    bugun = datetime.now()
    # Garanti olması için 30 günlük tarama yapıyoruz
    otuz_gun_once = bugun - timedelta(days=30)
    bir_yil_once = bugun - timedelta(days=365)
    bir_yil_otuz_gun_once = bir_yil_once - timedelta(days=30)
    
    try:
        # 1. Güncel Veri Çekimi
        guncel = tefas.fetch(start=otuz_gun_once.strftime("%Y-%m-%d"), end=bugun.strftime("%Y-%m-%d"), columns=["code", "title", "price", "market_cap", "number_of_investors"], kind="YAT")
        if guncel is None or guncel.empty or 'date' not in guncel.columns:
            raise ValueError("TEFAS şu an yanıt vermiyor. Lütfen daha sonra tekrar deneyin.")
            
        guncel['date'] = pd.to_datetime(guncel['date'])
        guncel_df = guncel[guncel['date'] == guncel['date'].max()].copy()
        son_tarih = guncel['date'].max() # Ekrana basmak için son tarihi alıyoruz
        
        # 2. Eski Veri Çekimi
        eski = tefas.fetch(start=bir_yil_otuz_gun_once.strftime("%Y-%m-%d"), end=bir_yil_once.strftime("%Y-%m-%d"), columns=["code", "price"], kind="YAT")
        if eski is not None and not eski.empty and 'date' in eski.columns:
            eski['date'] = pd.to_datetime(eski['date'])
            eski_df = eski[eski['date'] == eski['date'].max()].copy().rename(columns={"price": "eski_fiyat"})
        else:
            eski_df = pd.DataFrame(columns=["code", "eski_fiyat"])
        
        # 3. Portföy Dağılımı
        dagilim = tefas.fetch(start=otuz_gun_once.strftime("%Y-%m-%d"), end=bugun.strftime("%Y-%m-%d"), kind="YAT", columns=["code", "stock", "foreign_stock", "precious_metals"])
        if dagilim is not None and not dagilim.empty and 'date' in dagilim.columns:
        ...
        df = pd.merge(guncel_df, eski_df[['code', 'eski_fiyat']], on='code', how='left')
...
if market_choice == "Borsa İstanbul (BIST)":
    ...
    filters = [
        ...
        {"expression": {"left": "price_book_ratio", "operation": "in_range", "right": [0.1, pb_limit]}},
    ...
    if data:
        results = [{"Hisse": d['d'][0].replace("BIST:", ""), "Fiyat (TL)": d['d'][1], "F/K": round(d['d'][2],2) if d['d'][2] else "-", "PD/DD": round(d['d'][7],2) if d['d'][7] else "-", "ROE (%)": round(d['d'][3],2) if d['d'][3] else "-", "Temettü (%)": round(d['d'][4],2) if d['d'][4] else 0, "Borç/Özk.": round(d['d'][5],2) if d['d'][5] else "-", "PD (Milyar TL)": round(d['d'][6]/1e9, 2) if d['d'][6] else "-", "Sektör": d['d'][8]} for d in data]
    ...
    if data:
        results = [{"Hisse": d['d'][0], "Fiyat ($)": d['d'][1], "F/K": round(d['d'][2],2) if d['d'][2] else "-", "PD/DD": round(d['d'][7],2) if d['d'][7] else "-", "ROE (%)": round(d['d'][3],2) if d['d'][3] else "-", "Temettü (%)": round(d['d'][4],2) if d['d'][4] else 0, "Borç/Özk.": round(d['d'][5],2) if d['d'][5] else "-", "PD (Milyar $)": round(d['d'][6]/1e9, 2) if d['d'][6] else "-", "Sektör": d['d'][8]} for d in data]
        st.dataframe(pd.DataFrame(results), use_container_width=True)
    else:
    ...
    with st.spinner('TEFAS verileri taranıyor (Uzun tatillerde bu işlem biraz sürebilir)...'):
        try:
            tefas_df, son_tarih = fetch_tefas_data()
            st.success(f"✅ Veriler başarıyla çekildi. TEFAS'ın sunduğu en son veri tarihi: **{son_tarih.strftime('%d.%m.%Y')}**")
